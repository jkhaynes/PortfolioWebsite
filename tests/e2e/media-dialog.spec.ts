...
test("media dialog opens the selected project image and restores focus", async ({
  ...
  const pokeJudgeTrigger = page.getByRole("button", {
    name: "View larger: Complete PokéJudge clarification run",
  });
  ...
  const dialog = page.getByRole("dialog", {
    name: "Complete PokéJudge clarification run",
  });
  ...
  test(`media layouts avoid page overflow at ${viewport.width}px`, async ({
    ...
    for (const path of [
      ...
      await expect
        ...
        .toBe(true);
      // Homepage cards link to their case studies; screenshots open there.
      if (path === "/") continue;
...
test("featured project cards link to case studies and keep one preloaded image", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(cards.nth(0).getByRole("img")).not.toHaveAttribute(
    "loading",
    "lazy",
  );
  await expect(cards.nth(1).getByRole("img")).toHaveAttribute("loading", "lazy");
});
